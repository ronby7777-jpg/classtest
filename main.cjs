...
let dragging=null,petHidden=false;
...
function place(reset=false){if(!pet)return;const b=area();x=reset?(config.placement?Math.max(b.left,Math.min(b.right,config.placement.x)):(b.left+b.right)/2):Math.max(b.left,Math.min(b.right,x));pet.setBounds({x:Math.round(x),y:b.y,width:b.size,height:b.size});}
function moveDrag(){
  if(!dragging)return;
  const cursor=screen.getCursorScreenPoint(),dx=cursor.x-dragging.cursor.x,dy=cursor.y-dragging.cursor.y;
  if(!dragging.moved&&Math.hypot(dx,dy)<5)return;
  dragging.moved=true;
  const display=screen.getDisplayNearestPoint(cursor),work=display.workArea;
  const size=Math.min(config.size,work.width,work.height);
  x=Math.round(Math.max(work.x,Math.min(work.x+work.width-size,dragging.origin.x+dx)));
  const y=Math.round(Math.max(work.y,Math.min(work.y+work.height-size,dragging.origin.y+dy)));
  config.displayId=display.id;config.placement={x,y};pet.setBounds({x,y,width:size,height:size});
}
function finishDrag(){
...
function tick(){const now=Date.now(),dt=Math.min(.1,(now-last)/1000);last=now;if(dragging){moveDrag();return;}if(paused){if(mode==='pat'&&now>=until)action('idle',2000);return;}
...
app.whenReady().then(()=>{
  ...
  pet=new BrowserWindow(windowOptions({width:150,height:150,frame:false,transparent:true,alwaysOnTop:config.alwaysOnTop,resizable:false,skipTaskbar:true,hasShadow:false,focusable:true}));
  pet.setAlwaysOnTop(config.alwaysOnTop,'screen-saver');pet.loadFile(path.join(__dirname,'ui/pet.html'));place(true);
